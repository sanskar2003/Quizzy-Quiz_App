...
class gk_QueS : AppCompatActivity(), View.OnClickListener {
    ...
    var cp: Int = 0
    private val gkQuestions = Constants.getGkQuestions()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_gk_ques)
        ...
        submit.setOnClickListener{
            if(selectedOption==0){
                ...
                optionsView()
                cp++;
                if(cp == gkQuestions.size-1){
                    submit.text = "FINISH"
                }
                if(cp==gkQuestions.size){
                    cp = 0
    ...
    public fun setQuestion(){
        val Question: Questions = gkQuestions[cp]
