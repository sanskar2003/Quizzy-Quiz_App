...
class chem_QueS : AppCompatActivity(), View.OnClickListener {
    ...
    var cp: Int = 0
    private val chemQuestions = Constants.getChemQuestions()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_chem_ques)
        ...
        submit.setOnClickListener{
            if(selectedOption==0){
                ...
                optionsView()
                cp++;
                if(cp == chemQuestions.size-1){
                    submit.text = "FINISH"
                }
                if(cp==chemQuestions.size){
                    cp = 0
    ...
    public fun setQuestion(){
        val Question: Questions = chemQuestions[cp]
