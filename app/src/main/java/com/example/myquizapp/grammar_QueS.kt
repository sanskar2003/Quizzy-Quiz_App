...
class grammar_QueS : AppCompatActivity(), View.OnClickListener {
    ...
    var cp: Int = 0
    private val grammarQuestions = Constants.getGrammarQuestions()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_grammar_ques)
        ...
        submit.setOnClickListener{
            if(selectedOption==0){
                ...
                optionsView()
                cp++;
                if(cp == grammarQuestions.size-1){
                    submit.text = "FINISH"
                }
                if(cp==grammarQuestions.size){
                    cp = 0
    ...
    public fun setQuestion(){
        val Question: Questions = grammarQuestions[cp]
