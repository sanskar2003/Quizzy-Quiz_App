...
class ss_QueS : AppCompatActivity(), View.OnClickListener {
    ...
    var cp: Int = 0
    private val ssQuestions = Constants.getSSQuestions()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_ss_ques)
        ...
        submit.setOnClickListener{
            if(selectedOption==0){
                ...
                optionsView()
                cp++;
                if(cp == ssQuestions.size-1){
                    submit.text = "FINISH"
                }
                if(cp==ssQuestions.size){
                    cp = 0
    ...
    public fun setQuestion(){
        val Question: Questions = ssQuestions[cp]
